...
int main(int argc, char** argv)
{
	...
	if (!cap.read(image))
	{
        ...
        sensor_msgs::ImagePtr rawMsg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", std::move(image)).toImageMsg();
        rawPub.publish(rawMsg);

	auto now = Clock::to_time_t(Clock::now());
	cv::putText(image, std::ctime(&now), {20, 40}, font, 1, {0, 0, 0}, 8, false);
	cv::putText(image, std::ctime(&now), {20, 40}, font, 1, {255, 255, 255}, 4, false);

        sensor_msgs::ImagePtr stampedMsg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", std::move(image)).toImageMsg();
